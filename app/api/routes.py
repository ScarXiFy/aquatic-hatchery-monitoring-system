...
@api_bp.get("/readings/trend")
def readings_trend():
    """
    Returns readings from yesterday, or the most recent previous day
    that has data, along with the date those readings are from.
    """
    readings, date = get_yesterday_or_last_day()
    return jsonify({
        "readings": readings,
